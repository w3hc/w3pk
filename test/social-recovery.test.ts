...
{
  const secret = 'wide bundle detect art van embody ride human will leave brush angle';
  ...
  console.assert(recoveredSecret === secret, 'Should recover original secret');
...
console.log('\nTest 4: Hex Encoding and Decoding');
{
  ...
  console.assert(hex === '0102030405ff8040', 'Hex encoding should be correct');

  const decoded = hexToBytes(hex);
  console.assert(decoded.length === testData.length, 'Decoded should have same length');
  console.assert(decoded.every((byte, i) => byte === testData[i]), 'Decoded should match original');
...
console.log('\nTest 5: Social Recovery Setup');
{
  ...
  const testMnemonic = 'wide bundle detect art van embody ride human will leave brush angle';
  ...
  console.assert(result[0].name === 'Alice', 'Should preserve guardian names');
  console.assert(result[0].email === 'alice@example.com', 'Should preserve guardian emails');
  console.assert(result[0].id, 'Each guardian should have ID');
  console.assert(result[0].shareEncrypted, 'Each guardian should have encrypted share');
  console.assert(result[0].status === 'pending', 'Initial status should be pending');
...
console.log('\nTest 6: Guardian Invite Generation');
{
  ...
  console.assert(invite.guardianId === result[0].id, 'Invite should have correct guardian ID');
  ...
  console.assert(parsedShare.guardianId === result[0].id, 'Share should have guardian ID');
  console.assert(parsedShare.guardianName === 'Alice', 'Share should have guardian name');
  console.assert(parsedShare.threshold === 2, 'Share should have threshold');
  console.assert(parsedShare.totalGuardians === 3, 'Share should have total guardians');
...
console.log('\nTest 7: Recovery from Guardians');
{
  ...
  // Recover from shares
  const { mnemonic, ethereumAddress } = await socialRecovery.recoverFromGuardians([
    invite1.shareCode,
    invite2.shareCode,
    invite3.shareCode
  ]);

  console.assert(mnemonic === testMnemonic, 'Recovered mnemonic should match original');
...
console.log('\nTest 9: Recovery Progress Tracking');
{
  ...
  const progress1 = socialRecovery.getRecoveryProgress([invites[0].shareCode]);
  ...
  console.assert(progress2.canRecover, 'Should be able to recover with 2 shares');
...
console.log('\nTest 10: Guardian Verification');
{
  ...
  console.assert(verifiedGuardian?.status === 'active', 'Guardian should be active');
  console.assert(verifiedGuardian?.lastVerified, 'Should have lastVerified timestamp');
...
console.log('\nTest 11: Guardian Revocation');
{
  ...
  console.assert(revokedGuardian?.status === 'revoked', 'Guardian should be revoked');
...
console.log('\nTest 12: Storage Persistence');
{
  ...
  console.assert(config?.guardians.length === 2, 'Guardians should be persisted');
  console.assert(config?.ethereumAddress === testAddress, 'Address should be persisted');
